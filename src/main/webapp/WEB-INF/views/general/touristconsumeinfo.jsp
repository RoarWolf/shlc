<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html lang="en">
<head>
<meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
<meta charset="UTF-8">
<title>用户在线消费</title>
<%@ include file="/WEB-INF/views/public/compWechatFont.jspf"%>
<style>
@font-face {font-family: "iconfont";
  src: url('//at.alicdn.com/t/font_1221168_a2sbmjqs6uf.eot?t=1559285176791'); /* IE9 */
  src: url('//at.alicdn.com/t/font_1221168_a2sbmjqs6uf.eot?t=1559285176791#iefix') format('embedded-opentype'), /* IE6-IE8 */
  url('data:application/x-font-woff2;charset=utf-8;base64,') format('woff2'),
  url('//at.alicdn.com/t/font_1221168_a2sbmjqs6uf.woff?t=1559285176791') format('woff'),
  url('//at.alicdn.com/t/font_1221168_a2sbmjqs6uf.ttf?t=1559285176791') format('truetype'), /* chrome, firefox, opera, Safari, Android, iOS 4.2+ */
  url('//at.alicdn.com/t/font_1221168_a2sbmjqs6uf.svg?t=1559285176791#iconfont') format('svg'); /* iOS 4.1- */
}

.iconfont {
  font-family: "iconfont" !important;
  font-size: 16px;
  font-style: normal;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
}

.icon-laiyuan2:before {
  content: "\e77a";
}

.icon-consumption:before {
  content: "\e6f1";
}

.icon-yonghu-tianchong:before {
  content: "\e670";
}

.icon-icon:before {
  content: "\e611";
}

.icon-shijian:before {
  content: "\e681";
}

.icon-zhuangtai:before {
  content: "\e632";
}


		* {
			padding: 0;
			margin: 0;
		}
		li {
			list-style: none;
		}
		body {
			background-color: #efeff4;
		}
		header h3 {
			text-align: center;
			padding: 5px 0 10px;
			font-size: 14px;
		}
		.noData {
			position: absolute;
			left: 0;
			top: 40px;
			right: 0;
			bottom: 0;
			text-align: center;
			margin-top: 50%;
			color: #999;
			font-size: 14px;
		}
		.userDetail {
			padding: 15px 10px;
			color: #666;
			font-size: 12px;
		}
		.userDetail ul li {
			border: 1px solid #ccc;
			overflow: hidden;
			padding:10px;
			border-radius: 5px;
			background-color: #f5f7fa;
			margin-bottom: 15px;
		}
		.userDetail ul li:active {
			background-color: #EfEfEf;
		}
		.userDetail ul li div{
			min-width: 50%;
			float: left;	
		}
		.userDetail ul li div:nth-child(1){
			width: 100%;
		}
		
		.userDetail ul li div span {
			/*display: block;*/
			line-height: 30px;
			text-align: center;
		}
		.userDetail ul li div .title{
			color: #999;
		}
		.userDetail ul li div span i {
			color: #22B14C;
			font-size: 15px;
			margin-right: 5px;
		}
		
		.spanJust {
			color: #22B14C !important;
		}
		.spanNegative{
			color: #F47378 !important;
		}
		.bottomTip {
			text-align: center;
			color: #666;
			font-size: 14px;
			padding: 10px 0;
		}
	</style>
</head>
<body>
	<div class="userDetail">
		<header>
			<h3>用户消费</h3>
		</header>
		<ul>
			<c:choose>
			  <c:when test="${size==0}">
			  		<div class="noData">暂无数据</div>
			  </c:when>
			  <c:otherwise>
				<c:forEach items="${consumeinfo}" var="consume">
					<li>
						<div class="left">
							<span class="title"><i class="iconfont icon-icon" style="font-size: 14px;"></i>订单号：</span>
							<span>${consume.ordernum}</span>
						</div>
						<div class="left">
							<span class="title"><i class="iconfont icon-yonghu-tianchong"></i>用户：</span>
							<span>${consume.username}</span>
						</div>
						<div class="left">
							<span class="title"><i class="iconfont icon-zhuangtai"></i>状态：</span>
							<span>
							  <c:choose>
							    <c:when test="${consume.paysource==1 || consume.paysource==7}">充值</c:when>
							    <c:when test="${consume.paysource==5}">退费到钱包</c:when>
							    <c:when test="${consume.paysource==2 || consume.paysource==3 || consume.paysource==4}">消费</c:when>
							    <c:when test="${consume.paysource==6 || consume.paysource==8}">退款</c:when>
							  </c:choose>
							</span>
						</div>
						<div class="right">
							<span class="title"><i class="iconfont icon-consumption"></i>
							${consume.paysource==1 ? "充值消费" : consume.paysource==2? "充电消费" : consume.paysource==3 ? "投币消费" : 
							consume.paysource==4 ? "离线充值消费" : consume.paysource==5 ? "退款入钱包" : consume.paysource==6 ? "钱包退款" :
							consume.paysource==7 ? "虚拟充值" : consume.paysource==8 ? "虚拟退款" :  "--"} ：</span>
							<c:if test="${consume.paysource==1 || consume.paysource==5 || consume.paysource==7}">
							<span class="spanJust" >+<fmt:formatNumber value="${consume.money}" pattern="0.00" /></span>
							</c:if>
							<c:if test="${consume.paysource==2 || consume.paysource==3 || consume.paysource==4 || consume.paysource==6|| consume.paysource==8}">
							<span class="spanNegative" >-<fmt:formatNumber value="${consume.money}" pattern="0.00" /></span>
							</c:if>
							<strong>元</strong>
						</div>
						<div class="left">
							<span class="title"><i class="iconfont icon-zhuangtai"></i>充值余额：</span>
							<span><fmt:formatNumber value="${consume.topupbalance}" pattern="0.00"/>元</span>
						</div>
						<div class="left">
							<span class="title"><i class="iconfont icon-zhuangtai"></i>赠送余额：</span>
							<span><fmt:formatNumber value="${consume.sendbalance}" pattern="0.00"/>元</span>
						</div>
						<div class="right">
							<span class="title"><i class="iconfont icon-consumption" ></i>余额：</span>
							<span><fmt:formatNumber value="${consume.balance}" pattern="0.00"/></span>
							<strong>元</strong>
						</div>
						<c:if test="${consume.remark != null}">
						  <div class="left">
							<span class="title"><i class="iconfont icon-zhuangtai" ></i>备注：</span>
							<span>${consume.remark}</span>
						  </div>
						</c:if>
						<div class="right">
							<span class="title"><i class="iconfont icon-shijian" style="font-size: 17px;"></i>支付时间：</span>
							<span><fmt:formatDate value="${consume.create_time}" pattern="yyyy-MM-dd HH:mm:ss" /></span>
						</div>
					</li>
				</c:forEach>
			  </c:otherwise>
			</c:choose>
		</ul>
	</div>
	<div class="bottomTip">仅支持查看最近6个月的记录</div>
</body>
</html>