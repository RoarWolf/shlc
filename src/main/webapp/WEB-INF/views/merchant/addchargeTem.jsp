...
<head>
   ...
   <%@ include file="/WEB-INF/views/public/compWechatFont.jspf"%>
    ...
    <style>
    	...
    	.tem .mui-bar-tab a.goBack {
    		width: 60%;
    	}
    </style>
</head>
<!-- data-reload是告诉tem.js中添加主模板之后是否需要重新加载 1是需要重新加载 -->
<body class="tem1"  data-arecode="${arecode}"  data-source="${source}"  data-reload="1"> 
	<div class="tem">
<c:choose>
	<c:when test="${fn:length(templatelist) > 0 || fn:length(tempgather) > 0}">
	...
	<c:if test="${tempparent.pitchon == 1 }">
            ...
            <li>
                <div class="title">
                    ...
                    <p class="pTextInfo pTextInfo${tempparent.id}" data-text="${tempparent.chargeInfo}">收费说明： <button type="button" class="mui-btn mui-btn-success tem-title-text" data-id="${tempparent.id}" data-id="${tempparent.id}">查看</button>	
                   	</p>
   				...
   				<div class="list-div gradTem">
					...
					<div class="list-div">
				        ...
				        <!-- 这一部分的动态数据的显示，用js控制，控制data-id的数据，控制 button是否加active -->
				     	<c:if test="${temggather.rank ==3 }" >
				     	   <li class="mui-table-view-cell bottom"></li>
				     	</c:if>
		      <c:if test="${temggather.rank == 3 }"></div></c:if>
   </c:forEach>
	</c:when>
	<c:otherwise>
      	<div class="addTemplateInfo">
      		<!-- 点击下方的"添加模板"来添加主模板 -->
      	</div>
    </c:otherwise>
</c:choose>
        <nav class="mui-bar mui-bar-tab">
            <div>
            	<!-- 当有模板的时候只显示a标签，当没有模板的时候两个都显示 -->
            	<c:choose>
            		<c:when test="${fn:length(templatelist) > 0 || fn:length(tempgather) > 0}">
            			<a href="${hdpath}/merchant/devicetemdata?code=${code}" class="goBack"><button type="button" class="mui-btn mui-btn-success">模板列表</button></a>
            		</c:when>
            		<c:otherwise>
        				<a href="${hdpath}/merchant/devicetemdata?code=${code}"><button type="button" class="mui-btn mui-btn-success ">模板列表</button></a>
...
大功率电动车充电时间智能动态计算，以实际为准。">查看</button> 
                ...
                <div class="inp radio-inp">
                    ...
                    <br/>
                    <label>
                        <input type="radio" name="isRef" value="1" id="isRefInp" checked />是
                    </label>
                ...
                <div class="inp radio-inp">
                    ...
                    <br/>
                    <label>
                        <input type="radio" name="isIfalipay" value="1" checked />是
                    </label>
                    <label>
                        <input type="radio" name="isIfalipay" value="0" />否
                    </label>
                </div>
                 <div class="inp radio-inp">
                    <h5>是否创建分等级模板</h5>
