...
<%-- 				<div class="payType">
						...
						<span>
							<span id="paySpan">
								<c:if test="${ifwallet == 2 }">
									...
									钱包支付 （¥<span class="walletMoney">${balance}</span>）
									</c:if>
									<c:if test="${balance < 5 }">
					...
					<div class="monthPay">
						<span>包月剩余</span>
						<span>
							总剩余：${packageMonth.surpnum}次&nbsp;&nbsp;&nbsp;今日：${packageMonth.todaysurpnum}次
						</span>
					</div>
				</div>
			</div>
			<div class="paySelect animated slideInUp">
				<div class="title">
				<div class="back"><i class="mui-icon mui-icon-back"></i></div>
				选择支付方式
				</div>
				<ul>
					<li 
					<c:if test="${ifwallet == 2 && balance <5}">
						class="active"
					</c:if>
					<c:if test="${ifwallet != 2 }">class="firbid"</c:if>
					 data-pay-type="1">
						<i class="iconfont icon-weixin"></i>
						微信支付
					...
					<li 
					<c:if test="${ifwallet != 2 }">class="active"</c:if>
					<c:if test="${ifwallet == 2 && balance >= 5}">
						class="active"
					</c:if>
					>
						<i class="iconfont icon-qianbao" data-pay-type="2"></i>
						钱包支付（¥<span class="walletMoney">${balance}</span>元）
						<i class="mui-icon mui-icon-checkmarkempty"></i>
					</li>
					<div class="tip">
		...
		$(function(){
			...
			/* $('.slideDiv .select button').click(function(){
				...
				if($(this).attr('data-type') && $(this).attr('data-type').trim()==='monthCharge'){
					...
					if($('#paySpan i').hasClass('icon-qianbao')){ //这是判断当前状态下，支付方式是哪一个
						payTypeNum= 2
					}else{
						payTypeNum= 1
					}
