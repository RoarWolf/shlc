...
<body class="pcTem1 tem2"  data-arecode="${arecode}"  data-source="${source}"   data-merid="${merid}" >
  ...
  <li><a href="javascript:void(0)" target="right" class="icon-home">模板查看</a></li>
	...
	<div class="admin">
		...
		<div class="temDiv tem1">
			<div class="top">
				<h3>离线卡模板 <button type="button" class="btn btn-info addTemplate"  data-tem="1" >添加主模板</button></h3>
			</div>
			...
			$('.admin').click(function (e) {
			           ...
			           $.ajax({  //添加子模板
			               ...
			               success : function(e){
			                   if(e.code=== 100){
			                       ...
			                       var str= '<td>'+nextName+'</td> <td><span>'+nextParse+'</span>元</td><td><span>'+nextTotalParse+'</span>元</td><td class="setWidth"><button type="button" class="btn btn-info tem-edit" data-id="'+e.ctemid+'">编辑</button></td><td class="setWidth"><button type="button" class="btn btn-danger tem-delete" data-id="'+e.ctemid+'" '+disableStr+' >删除</button></td>'
			                      var list= $('<tr></tr>')
			                      list.html(str)
			       ...
			       /*else if( $(target).hasClass('addTemplate')){ //添加主模板
				 	...
				 	$('.tem2 .list-center1').mousedown(function(e){
					   ...
					   $('.tem2 .list-center2 .submit').click(function(e){ //添加或修改主模板
					        e =e || window.event
					        e.stopPropagation()
					        ...
					        if(flag){ // 修改离线模板
					            ...
					            //发送ajax将新增的数据传输到服务器=====================
					             ...
					             $.ajax({
					                ...
					                url : "/wctemplate/allowaddstairoffline",
					                type : "POST",
					                cache : false,
					                success : function(e){
					                	if(e.code === 101){
