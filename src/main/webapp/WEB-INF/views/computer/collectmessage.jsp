...
<ul class="bread">
  <li><a href="javascript:void(0)" target="right" class="icon-home">汇总信息</a></li>
</ul>
<div class="admin">
  ...
  <div class="table table-div">
		<table class="table table-hover">
			<tbody>
				<tr class="headline"><th colspan="10">有效客户:</th></tr>
				<tr>
					<th>有效客户数</th><td>${ditequ}</td>
					<th></th><td></td>
					<th></th><td></td>
					<th></th><td></td>
					<th></th><td></td>
				...
				<tr>
					<th>设备总数</th><td>${equnum}</td>
					<th>设备在线总数</th><td>${online}</td>
					<th>设备离线总数</th><td>${equnum - online}</td>
					<th>设备绑定总数</th><td>${binding}</td>
					<th>设备未绑定总数</th><td>${equnum - binding}</td>
				</tr>
				...
				<tr>
					<th>今日金额</th><td>${totalNowmap.totalmoney}</td>
					<th>今日在线收款</th><td>${totalNowmap.onlinemoney}</td>
					<th>今日在线退款</th><td>${totalNowmap.refonlinemoney}</td>
					<th></th><td></td>
					<th></th><td></td>
				</tr>
				<tr class="headline"><th colspan="10"><a href="${hdpath}/pcstatistics/statisticsinfo">历史统计</a></th></tr>
				<tr class="headline"><th colspan="10" style="color: #da391d;">
														       注：总订单信息为绑定设备后的在线信息；今日订单信息为当天收入信息</br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;资金总额为在线收款、在线退款和提现金额的总计</br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;提现金额为已从平台提取的金额；提现费用为微信提现收取的费用
				</th></tr>
			</tbody>
		</table>
	</div>
...
$(document).ready(function(){	
	$('#11'+' a').addClass('at');
	$('#11').parent().parent().parent().prev().removeClass("collapsed");
	$('#11').parent().parent().parent().prev().attr("aria-expanded",true)
	$('#11').parent().parent().parent().addClass("in");
	$('#11').parent().parent().parent().prev().attr("aria-expanded",true)
	})
</script>
</html>