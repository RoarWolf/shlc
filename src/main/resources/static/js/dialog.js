(function($){
	$.fn.dailog = function(width,  height, title, htmlText, options,callBack){
	    var _this = this;
	    var defaultDailog = {
	      width:              width,                      //宽度
	      height:             height,                     //高度
	    ...
	    var _isScroll = function(){//（遮罩）隐藏原页面
	    	if(opt.hideScroll){
	    		$('body,html').css({
		    		overflow:'hidden',
		    	});
	    ...
	    var _colseScroll = function(){//页面信息如果查过，滚动查询
	    	$('body,html').css({
	    		overflow:'auto',
	    	});
	    }

	    var _overflowBtn = function(){//判断按钮有几个
	    	// bottons超过两个提示
	    ...
	    var _isBoxShadow = function(){//弹窗是否显示boxshadow
	    	// 是否显示boxshadow
	    ...
	    var _btnIndex = function(name){//索引
	    	//获取点击的索引
	      	var btnName = name || '';
